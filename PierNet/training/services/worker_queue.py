"""Training task queue consumed by the PierNet worker."""
...
QUEUE_LOCK_TTL_SECONDS = float(os.getenv("PierNet_WORKER_QUEUE_LOCK_TTL_SECONDS", "60"))
...
def run_next_queued_job(*, worker_id: str | None = None) -> bool:
    ...
    try:
        ...
        for job in queued:
            ...
            try:
                workers.upsert_worker(worker_id=owner, kind="PierNet-worker", status="running", current_job_id=job_id)
                ...
                with task_locks.refresh_lock_while(
                    ...
                    worker_id=owner, kind="PierNet-worker", current_job_id=job_id, interval=5.0
                ):
                ...
                workers.upsert_worker(worker_id=owner, kind="PierNet-worker", status="running", current_job_id=None)
    finally:
