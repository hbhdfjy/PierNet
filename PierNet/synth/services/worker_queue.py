...
from PierNet.synth.services import generation_executor, job_manager, job_store, router_executor
...
def queue_enabled() -> bool:
    return os.getenv("PierNet_WORKER_QUEUE_SYNTH", "1").strip().lower() not in {"0", "false", "no", "off"}
        ...
        for stored in _queued_jobs():
            ...
            try:
                with task_locks.refresh_lock_while(
                    ...
                    worker_id=owner, kind="PierNet-worker", current_job_id=record.job_id
                ):
