...
用法：
  ...
  # 只注册指定场景
  python -m PierNet.synth.text2comp.auto_register --scenarios unified_aquifer ieee14_baseload

  # 只注册某字段组
  python -m PierNet.synth.text2comp.auto_register --fields domain
  python -m PierNet.synth.text2comp.auto_register --fields output_info
  python -m PierNet.synth.text2comp.auto_register --fields observation

  # simulator 级别注册通用字段（output_info/observation_config 各场景相同时推荐）
  python -m PierNet.synth.text2comp.auto_register --fields output_info observation --simulator-level

  # 再按场景注册特有字段（domain_context/param_info 各场景不同）
  python -m PierNet.synth.text2comp.auto_register --fields domain

  # 组合：指定场景 + 指定字段组
  python -m PierNet.synth.text2comp.auto_register --scenarios unified_aquifer --fields observation

  # 强制覆盖已有字段
  python -m PierNet.synth.text2comp.auto_register --overwrite
"""
...
from PierNet.synth.text2comp.pipeline import load_config, _scenario_name_from_path, _scan_h5_files
