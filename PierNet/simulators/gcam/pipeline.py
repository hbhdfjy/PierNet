...
from PierNet.simulators.gcam.generator import generate_batch
from PierNet.simulators.gcam.generator_with_params import generate_batch_from_params
from PierNet.simulators.gcam.unified_params import GCAMParamConverter
...
def _top_up_direct_samples(
    ...
    while sum(x.shape[0] for x in batches_ts) < n_samples:
        ...
        batch_seed += batch_n
        extra_ts, extra_params, _ = filter_gcam_dataset(extra_ts, extra_params)
    ...
    if all_ts.shape[0] < n_samples:
        raise RuntimeError(f'GCAM 有效样本不足，仅得到 {all_ts.shape[0]}/{n_samples} 条')
...
def run_pipeline(
    ...
    if aug_ts.shape[0] < n_samples:
        raise RuntimeError(f'GCAM 有效样本不足，仅得到 {aug_ts.shape[0]}/{n_samples} 条')
