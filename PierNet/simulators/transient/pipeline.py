...
from PierNet.simulators.transient.generator import generate_batch, _generate_single_worker
from PierNet.simulators.transient.generator_with_params import generate_batch_from_params
from PierNet.simulators.transient.unified_params import TransientParamConverter
from PierNet.core.validation import filter_dataset
...
def _top_up_direct_samples(
    ...
    if all_ts.shape[0] < n_samples:
        raise RuntimeError(f'暂态有效样本不足，仅得到 {all_ts.shape[0]}/{n_samples} 条')
...
def run_pipeline(
    ...
    if timeseries.shape[0] >= n_samples:
        logger.info('Step 3/5: 直接采样已满足配置数量，跳过增强')
        aug_ts = timeseries[:n_samples]
        aug_params = params[:n_samples]
    elif seed_ratio >= 1.0 or not aug_cfg.get('enabled', True):
        logger.info('Step 3/5: 直接采样不足，继续补充真实样本')
        aug_ts, aug_params = _top_up_direct_samples(
            cfg, timeseries, params, n_samples, seed, parallel, max_workers, progress_callback=progress_callback
        )
    else:
    ...
    if aug_ts.shape[0] < n_samples:
        raise RuntimeError(f'暂态有效样本不足，仅得到 {aug_ts.shape[0]}/{n_samples} 条')
