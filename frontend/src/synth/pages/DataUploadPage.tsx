...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-700/35 bg-slate-900/30 p-3">
      ...
      <div className="truncate font-mono text-sm font-semibold text-slate-100" title={value}>{value}</div>
    </div>
  )
}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {file && (
                      <div className="mt-3 text-sm text-slate-400">
                        已选择 <span className="inline-block max-w-full truncate align-bottom font-mono text-slate-200" title={file.name}>{file.name}</span>，大小 {formatBytes(file.size)}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-sky-500/20 bg-sky-500/8 p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="mt-1 truncate font-mono text-sm text-sky-300" title={targetPath}>{targetPath}</div>
                  </div>
                </div>
              </div>
