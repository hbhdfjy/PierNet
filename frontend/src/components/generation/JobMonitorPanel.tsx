...
const ACCENT = {
  ...
  emerald: { bar: 'from-emerald-500 to-emerald-400', dot: 'bg-emerald-400', ring: '#10b981', glow: 'shadow-glow-emerald' },
...
function ScenarioCard({
  ...
  scenario: string; done: number; total: number; isRunning: boolean; accent: string
}) {
...
function OverallRing({
  ...
  totalDone: number; totalTarget: number; status: JobStatus; accent: string
}) {
      ...
      <div className="relative w-20 h-20">
        <svg className="w-20 h-20 -rotate-90" viewBox="0 0 80 80">
          <circle cx="40" cy="40" r={r} fill="none"
            stroke="rgba(51,65,85,0.5)" strokeWidth="5" />
          <circle
          ...
          {logsOpen && (
            <div style={{ height: '220px', borderTop: '1px solid hsl(var(--border) / 0.4)' }}>
              <Virtuoso
                ...
                itemContent={(_, log) => (
                  <div className={cn(
                    'px-4 py-0.5 font-mono text-xs leading-5 select-text',
                    LOG_COLOR(log.line),
                  )}>
                    {log.line}
                  </div>
