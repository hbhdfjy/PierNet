...
当前工业化完成度约为 88% 到 92%。后续升级的核心不应再是补齐单机 worker，而应围绕可观测性、最小闭环测试、严格类型质量、安全治理和长期数据治理逐步推进。

### 1.1 2026-05-17 执行顺序修订

本轮将工业化推进顺序明确为：先完成部署守护、环境配置、迁移验收和仓库卫生四项 P0 基线，再推进 worker 化、统一任务状态机和数据库迁移等 P1 架构改造。原因是 worker 化属于任务执行层大改，如果在服务守护和迁移验收之前推进，会放大排障和回滚成本。

本轮先落地：

1. 用户级 systemd 服务模板和安装脚本。
2. `.env.example` 全覆盖基础审计。
3. `check_migration_ready.py` 迁移验收脚本。
4. `check_repo_hygiene.py` 仓库卫生脚本。
5. CI 中接入上述轻量检查。
6. 统一配置校验、health ready 配置摘要和 systemd 接管收尾。

### 1.2 2026-05-17 P1/P2 基础设施落地记录

本轮已继续落地以下工业化基础能力：

1. 统一任务状态机：新增共享状态定义、合法转换校验和测试。
2. 统一任务锁：新增 SQLite 协作锁，接入样本填充、Router 构建和训练 GPU 占用。
3. 数据库迁移：新增轻量 `schema_migrations`，合成任务和训练任务 store 已接入版本化初始化。
4. Worker 进程：新增维护型 worker，用于清理过期锁，并为后续真正的队列 worker 预留 systemd 入口。
5. 当前定位为单用户自用平台，已移除应用层访问控制；公网部署前应重新评估认证边界。
...
- 当前适合远程服务器常驻开发和单机使用。
- 已具备用户级 systemd 基础部署层；Docker、Nginx 和生产静态前端流程仍需继续补齐。
...
- 服务可启动、停止、重启和查看状态。
- 已新增用户级 systemd 模板和安装脚本。
- 当前服务器已由 systemd 接管前后端服务，并启用 user linger。
...
- 已新增仓库卫生检查，阻止缓存、SQLite runtime、训练日志、权重和 Parquet 派生物进入 Git。
...
## 6. P1：任务执行器工业化（P0 基线完成后）

说明：任务执行器工业化从 P0 调整为 P1。前置条件是 systemd/生产入口、`.env` 配置、迁移验收和仓库卫生检查已经落地并进入 CI。
...
- 当前项目按单用户自用平台运行。
- 前端和后端不再内置登录态或写操作访问控制。
- 安全边界依赖服务器账号、SSH、端口开放策略和可信网络。

后续只有在对公网或多人开放时才需要重新设计：

1. 明确用户模型、角色和敏感操作范围。
2. 选择统一登录方案，而不是在界面中暴露临时凭据输入框。
3. 配套 CSRF、审计、日志脱敏和反向代理访问控制。

验收标准：

- 单用户本地/内网使用时界面没有多余登录和令牌组件。
- 若未来开放多人访问，必须先完成正式认证方案设计。
...
截至 2026-05-17，单机 worker 化、统一任务状态、统一任务锁、训练队列、合成队列、worker 心跳、统一任务 API、审计 API 和基础视觉回归已经落地。当前系统已经进入“工业化单机任务平台”阶段。

后续不应再重复做 worker 化，而应继续推进以下能力：

1. 任务可靠性：worker 长任务持续心跳、stale worker 告警、队列堆积告警和任务失联诊断。
2. 可观测性：统一 `/api/metrics/summary`，集中暴露任务、队列、worker、磁盘和 GPU 状态。
3. 最小闭环测试：构造小数据跑样本填充、Router 构建和 1 epoch 训练，作为 nightly smoke。
4. 存储治理：准备 PostgreSQL/Alembic、对象存储和模型注册表，但在单机阶段不强制引入。
5. 安全治理：若未来开放多人使用，再引入用户、角色、审计和危险操作确认。

当前最高优先级是“可观测性 + 最小闭环测试”，因为它们能降低后续重构时的回归风险。
